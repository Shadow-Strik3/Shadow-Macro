...
// -------------------------------------------------------------------------
// Your GitHub repository in "owner/repo" form. The app reads this repo's
// "latest release" to detect updates. The repo must be PUBLIC for the
// unauthenticated check to work. Override at runtime with SHADOW_GITHUB_REPO.
const GITHUB_REPO = process.env.SHADOW_GITHUB_REPO || 'Shadow-Strik3/Shadow-Macro';
...
// opts: { auto }  — when called automatically (startup), the caller passes
// auto:true and should only call this if updates are enabled. Manual checks
// from the UI always run regardless of the toggle.
