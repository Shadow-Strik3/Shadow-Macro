...
**How it works:** the app calls GitHub's public API for the repo's *latest
...
**Configured repo:** `Shadow-Strik3/Shadow-Macro` (set in `src/main/updater.js`,
overridable via the `SHADOW_GITHUB_REPO` env var).

**On/off toggle:** Settings → Updates has **Automatic updates** (master switch)
and **Check on startup**. When automatic updates are off, no background checks
run; the manual **Check for Updates** button still works on demand. These
preferences persist in the settings store.

> ⚠️ The repo **must be public** for the unauthenticated check to work. If it's
> private, GitHub returns 404 and the app simply reports "up to date" (no error).
